...
CLAUDE.mdに定義された世界No.1チームの各専門家として、review-rubric.md の採点基準に従い記事をレビューしてください。

【ステップ2-1：AI文章判定ゲート（必須）】
✍️ コピーライター＋📋 ディレクターとして、まず記事がAIが書いた文章に見えないかを判定する。
article-prompt.md の「絶対に使わない表現リスト」と「構造のAI臭パターン」を基準にチェックする。
1つでも該当したら「AI文章判定：NG」とし、即座に書き直す。OKになるまでこのステップを繰り返す。

【ステップ2-2：採点】
ゲート通過後、以下の担当者として採点する：
- 🚀 SEO/MEO/LLMエキスパート → 【A】SEO基本品質（20点）
- ✍️ コピーライター → 【B】コンテンツ・人間らしさ（20点）
